...
// Handle form submission para mag-add ng bagong vaccine
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_vaccine'])) {
    ...
    $sql = "INSERT INTO vaccines (vaccine_name, description, category) VALUES (?, ?, ?)";
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("sss", $vaccine_name, $description, $category);
        if ($stmt->execute()) {
...
<head>
    ...
    <style>
        ...
        /* Modal Styling for Adding Vaccine */
        .modal {
            display: none; 
            position: fixed; 
            z-index: 1000; 
                    ...
                    <?php if ($result_baby->num_rows > 0): ?>
                        <?php while($row = $result_baby->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><a href="#" style="color: var(--sage-green);">Edit</a></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="4" style="text-align:center;">No baby vaccines in inventory yet.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <?php if ($result_maternal->num_rows > 0): ?>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="4" style="text-align:center;">No maternal vaccines in inventory yet.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
<div id="addModal" class="modal">
    <div class="modal-content">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h3 style="margin: 0; text-transform: uppercase; font-size: 1rem;">Add Vaccine</h3>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <textarea name="description" rows="3" placeholder="Target disease or notes..."></textarea>
            </div>
